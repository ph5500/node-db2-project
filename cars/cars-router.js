...
router.get("/", (req, res) => {
    db.select("*")
        ...
        .catch(err => {
            console.log(err);
            res.status(500).json({ errorMessage: "The datat could not be retrieved." });
        });
});
...
router.post("/", (req, res) => {
    if (!req.body.make || !req.body.model || !req.body.vin || !req.body.mileage) {
        res.status(400).json({
            errorMessage: "Please provide the make, model, vin, and mileage for the car."
        });
    } else {
        db("cars")
            .insert(req.body, "id")
...
router.put("/:id", (req, res) => {
    ...
    db("cars")
        ...
        .catch(err => {
            console.log(err);
            res.status(500).json({ errorMessage: "The date could not be updated." })
        });
});
